-- =====================================================
-- MIGRATION COMPLÈTE MySQL vers Supabase (PostgreSQL)
-- Base de données Tbc Groupe
-- =====================================================
--
-- INSTRUCTIONS :
...
DO $$ 
BEGIN
    -- Supprimer le type s'il existe déjà (pour réexécution)
    DROP TYPE IF EXISTS user_role CASCADE;

-- Créer le type avec toutes les valeurs nécessaires
CREATE TYPE user_role AS ENUM ('super_admin', 'admin', 'editor');

EXCEPTION WHEN duplicate_object THEN
-- Si le type existe déjà, ajouter les valeurs manquantes
BEGIN
...
CREATE INDEX IF NOT EXISTS idx_messages_read_by ON contact_messages (read_by);
...
-- Vue: v_trainers_details (Formateurs avec statistiques)
...
-- Vue: v_unread_messages (Messages non lus)
...
-- =====================================================
-- FONCTIONS STOCKÉES (Stored Procedures)
-- =====================================================

-- Fonction: sp_mark_message_read (Marquer un message comme lu)
CREATE OR REPLACE FUNCTION sp_mark_message_read(
...
-- Fonction: sp_get_site_stats (Obtenir les statistiques du site)
CREATE OR REPLACE FUNCTION sp_get_site_stats()
RETURNS TABLE (
...
BEGIN
    RETURN QUERY
    SELECT 
        (SELECT COUNT(*) FROM trainers WHERE is_active = TRUE)::BIGINT as active_trainers,
        (SELECT COUNT(*) FROM services WHERE is_active = TRUE)::BIGINT as active_services,
        (SELECT COUNT(*) FROM portfolio_projects WHERE is_active = TRUE)::BIGINT as active_projects,
...
-- Insertion de l'administrateur super_admin par défaut
-- IMPORTANT: Le mot de passe doit être hashé avec password_hash() de PHP
-- Exemple: password_hash('votre_mot_de_passe', PASSWORD_DEFAULT)
INSERT INTO
    users (
        email,
        password_hash,
...
VALUES (
        ...
        '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', -- Hash par défaut (à changer)
...
INSERT INTO services (title, description, icon_name, features, technologies, is_active, display_order)
VALUES 
    (
        'Développement Web',
...
BEGIN
    ...
    AND table_name IN (
        'users', 'trainers', 'trainer_experiences', 'trainer_skills',
        'trainer_technologies', 'services', 'portfolio_projects',
        'contact_messages', 'site_settings', 'training_programs'
    );
    
    IF table_count = 10 THEN
        RAISE NOTICE '✅ Toutes les tables ont été créées avec succès!';
    ELSE
        RAISE WARNING '⚠️ Seulement % tables créées sur 10 attendues', table_count;
    END IF;
END $$;
